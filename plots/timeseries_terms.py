...
def _plotter(df, term_list, figures_directory):
    ...
    for i, term in enumerate(terms):
        if term in df.columns:
            plt.plot(times, df[term], c=utils.COLORS[i], label=term, lw=utils.LINEWIDTH, marker=utils.MARKERS[i], 
                 markersize=8, markerfacecolor=utils.COLORS[i], markeredgecolor=utils.MARKER_EDGE_COLOR, markeredgewidth=0.5, linestyle=utils.LINESTYLE)
...
def plot_timeseries(results_file, figures_directory, app_logger=False):
    ...
    for term_list in utils.TERM_DETAILS:
        ...
        app_logger.info(f'Figure saved for {term_list} in directory: {figures_subdirectory}') if app_logger else print(f'Figure saved for {term_list} in directory: {figures_subdirectory}')
