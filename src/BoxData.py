...
class BoxData:
    ...
    def __init__(self, data: xr.Dataset, variable_list_df: pd.DataFrame,
        ...
        self._set_domain_limits(data, western_limit, eastern_limit, southern_limit, northern_limit)
    ...
    def _set_domain_limits(self, data, western_limit, eastern_limit, southern_limit, northern_limit):
        ...
        self.southern_limit = self._select_nearest(data, self.LatIndexer, southern_limit)
        self.northern_limit = self._select_nearest(data, self.LatIndexer, northern_limit)

        self.xlength = self.eastern_limit['rlons'] - self.western_limit['rlons']
        self.ylength = np.sin(self.northern_limit['rlats']) - np.sin(self.southern_limit['rlats'])
    ...
    def _process_wind_data(self, data, variable_list_df):
        """Extract and process wind component data."""
        self.u = self._extract_data(data, variable_list_df, 'Eastward Wind Component', 'm/s')
        ...
        self.v = self._extract_data(data, variable_list_df, 'Northward Wind Component', 'm/s')
    ...
    def _process_friction_terms(self, data, variable_list_df, args):
        ...
        if args.residuals:
            ...
            self.ust = self._extract_data(data, variable_list_df, 'Friction Velocity', 'm/s')
            self.vst = self._extract_data(data, variable_list_df, 'Friction Velocity', 'm/s')
    ...
    def _process_omega(self, data, variable_list_df):
        ...
        self.omega = self._extract_data(data, variable_list_df, 'Omega Velocity', 'Pa/s')
    ...
    def _extract_and_convert_geopotential_height(self, data, variable_list_df):
        ...
        geopt_height = self._extract_data(data, variable_list_df, 'Geopotential Height', variable_list_df.loc['Geopotential Height']['Units'])
    ...
    def _extract_data(self, data, variable_list_df, variable_name, unit):
        ...
        return (data[var_key] * unit_to_convert).metpy.convert_units(unit).sel(
            **{self.LatIndexer: slice(self.southern_limit, self.northern_limit),
               self.LonIndexer: slice(self.western_limit, self.eastern_limit)})