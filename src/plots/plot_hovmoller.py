...
def _plotter(dict_vertical, figures_subdirectory, app_logger=False):
    ...
    for term_type, terms in all_terms.items():
        ...
        app_logger.info(f"Figure saved to {outfile}") if app_logger else print(f"Figure saved to {outfile}")
...
def plot_contour(times, levs, data, ax, term):
    ...
    cf = ax.contourf(times, levs, data[levs].transpose(), cmap=cmap, extend='neither', norm=norm)
    ...
    if term in utils.TERM_DETAILS['conversion']['terms'] or term in utils.TERM_DETAILS['generation_dissipation']['terms']:
...
def get_cmap_norm_title_for_term(term, imin, imax, absmax):
    if term in utils.TERM_DETAILS['energy']['terms']:
        ...
        cmap = 'cmo.tarn'
        norm = colors.TwoSlopeNorm(vcenter=0, vmin=-absmax, vmax=absmax)
        title = f'{utils.TERM_DETAILS["generation_dissipation"]["label"]} ({utils.TERM_DETAILS["generation_dissipation"]["unit"]})'
...
def plot_hovmoller(results_file, figures_directory, app_logger=False):    
    ...
    app_logger.info('Creating hovmoller diagrams') if app_logger else print('Creating hovmoller diagrams')
