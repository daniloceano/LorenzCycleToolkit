...
def plot_min_zeta_hgt(track_file, figures_directory, app_logger=False):
    ...
    try:
        ...
        app_logger.info('Error: Parsing CSV file failed. Check the file format.') if app_logger else None
        return

    plt.close('all')
    fig, ax1 = plt.subplots(figsize=(10, 8))
    
    ax1.plot(track.index, track['min_max_zeta_850'], '--', c=COLORS[0], lw=LINEWIDTH, label=r'$\zeta$')
    ax2 = ax1.twinx()
    ax2.plot(track.index, track['min_hgt_850'], '-', c=COLORS[1], lw=LINEWIDTH, label=r'$Z$')
